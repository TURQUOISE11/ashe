...
import com.amumu.ashe.user.entity.UserElement;
...
public class CommonCacheUtil {
	 ...
	 * @throws AsheException
	 */
	public int cacheForVerificationCode(String key, String value, String type, int timeout, String ip) throws AsheException {
		try {
			JedisPool pool = jedisPoolWrapper.getJedisPool();
			if (pool != null) {
			...
			throw new AsheException("Fail to cache for expiry");
		}
	 ...
	 * @throws AsheException
	 */
	public UserElement getUserByToken(String token) throws AsheException {
		UserElement ue = null;
		...
		if (pool != null) {
			try (Jedis jedis = pool.getResource()) {
				...
				try {
					...
					throw new AsheException("Fail to get token content");
				}
			}
		}
